...
var CorrespondingAuthor = module.exports.CorrespondingAuthor = function(entry) {
    var data = entry['api:relationship'][0]['api:related'][0]['api:object'][0];

    var author = {};
    author.isAcademic = data['api:is-academic'][0];
    author.isCurrentStaff = data['api:is-current-staff'][0];
    author.title = data['api:title'][0];
    author.initials = data['api:initials'][0];
    author.firstName = data['api:first-name'][0];
    author.lastName = data['api:last-name'][0];
    author.academicName = String(author.title + ' ' + author.initials + ' ' + author.lastName).trim();
    author.emailAddress = data['api:email-address'][0];
    author.primaryGroupDescriptor = data['api:primary-group-descriptor'][0];
    return author;
};
...
var Publication = module.exports.Publication = function(id, displayName, publicationType, source, date, authors, publisher, publicationStatus, extra) {
    ...
    Object.defineProperty(that, 'id', {'enumerable': false});
